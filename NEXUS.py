import asyncio,json,os,time,secrets,hashlib,base64
import websockets
...
async def sendto(uid,data):
    ...
    if not ok and online.get(str(uid))==ws:online.pop(str(uid),None)
    return ok
...
async def login(ws,d):
    ...
    print(f"[LOGIN] {uid} | ADMIN={u.get('admin',False)}")
    return uid

async def usuarios_lista(ws):
    await send(ws,{"tipo":"usuarios","usuarios":[perfil(x) for x in usuarios if perfil(x)]})

async def foto(uid,d,ws):
    foto=d.get("foto","")
    if not isinstance(foto,str):return await error(ws,"Foto inválida.")
    if len(foto)>MAX_FOTO:return await error(ws,"A foto é muito grande.","FOTO_GRANDE")
    if foto and not foto.startswith("data:image/"):return await error(ws,"Formato de foto inválido.")

    async with lock:
        if uid not in usuarios:return
        usuarios[uid]["foto"]=foto
        save(USERS,usuarios)

    p=perfil(uid)
    await send(ws,{"tipo":"perfil_atualizado","perfil":p})

    pacote={"tipo":"perfil_update","perfil":p}
    for x in list(online):
        if x!=uid:await sendto(x,pacote)

async def perfil_req(ws,d):
    uid=str(d.get("id","")).strip()
    if uid not in usuarios:return await error(ws,"Usuário não encontrado.")
    await send(ws,{"tipo":"perfil","perfil":perfil(uid)})

async def historico(ws,uid,d):
    outro=str(d.get("para","")).strip()
    if outro not in usuarios:
        return await send(ws,{"tipo":"historico","para":outro,"mensagens":[]})
    lista=mensagens.get(chave(uid,outro),[])[-1000:]
    await send(ws,{"tipo":"historico","para":outro,"mensagens":lista})

async def mensagem(uid,d):
    para=str(d.get("para","")).strip()
    if para not in usuarios:return await sendto(uid,{"tipo":"erro","mensagem":"Usuário não encontrado."})

    texto=d.get("texto","")
    if not isinstance(texto,str):texto=str(texto)
    if len(texto)>MAX_MSG:return await sendto(uid,{"tipo":"erro","mensagem":"Mensagem muito grande."})

    tipo=d.get("tipo_mensagem","texto")
    if tipo not in ("texto","imagem","gif","audio"):tipo="texto"

    mid=d.get("id") or f"{uid}_{int(time.time()*1000)}_{secrets.token_hex(4)}"

    try:timestamp=int(d.get("timestamp",0))
    except:timestamp=int(time.time()*1000)

    try:seq=int(d.get("seq",timestamp))
    except:seq=timestamp
    ...
    entregue=await sendto(para,{"tipo":"mensagem","mensagem":msg})
    await sendto(uid,{"tipo":"mensagem_enviada","mensagem":msg,"entregue":entregue})

def eh_admin(uid):
    u=usuarios.get(uid)
    return bool(u and u.get("admin",False) and not u.get("banido",False))
...
async def admin_acao(uid,d,ws):
    if not await exigir_admin(ws,uid):return

    alvo=str(d.get("id","")).strip()
    acao=d.get("acao")
    u=usuarios.get(alvo)

    if not u:return await error(ws,"Usuário não encontrado.")
    ...
    save(USERS,usuarios)
    p=perfil(alvo)
    await send(ws,{"tipo":"admin_ok","acao":acao,"perfil":p})

    for x in list(online):
        await sendto(x,{"tipo":"perfil_update","perfil":p})
...
async def handler(ws):
    ...
    try:
        async for raw in ws:
            ...
            except:await error(ws,"JSON inválido.");continue
...
async def main():
    ...
    async with websockets.serve(handler,HOST,PORT,max_size=20*1024*1024,ping_interval=20,ping_timeout=30):
